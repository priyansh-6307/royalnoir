...
const Portfolio = () => {
  ...
  return (
    <section id="portfolio" className="py-12 md:py-24 bg-black">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header Section */}
        <div className="text-center mb-12 md:mb-20">
          <div className="mb-6 md:mb-8">
            <div className="text-xs md:text-sm tracking-[0.3rem] md:tracking-[0.5rem] text-gray-400 uppercase">
              EXPLORE FURTHER
            </div>
            <h2 className="text-base md:text-lg tracking-[0.15em] md:tracking-[0.2em] text-gray-300 uppercase">
              Our Services
            </h2>
          </div>
        </div>
          ...
          {visibleServices.map((service) => (
            ...
            >
              ...
              <div className="p-4 md:p-8">
                <h3 className="text-base md:text-xl font-light tracking-[0.1em] text-white uppercase mb-3 md:mb-4 leading-tight">
                  {service.title}
                </h3>
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-white/20 text-black hover:bg-white hover:text-black transition-all duration-300 text-[0.65rem] md:text-xs tracking-wider uppercase"
                  >
                    More info
        ...
        <div className="text-center mt-8 md:mt-10">
          <Button
            variant="outline"
            className="text-black hover:bg-gray-200 hover:text-black transition-all duration-300 px-8 md:px-10 py-2 md:py-3 text-[0.65rem] md:text-xs tracking-wider uppercase"
        ...
        <div className="text-center mt-8 md:mt-10">
          <Button
            variant="outline"
            size="lg"
            className="border-white/30 text-black hover:bg-white hover:text-black px-10 md:px-12 py-3 md:py-4 text-xs md:text-sm tracking-[0.1em] uppercase transition-all duration-300"
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white text-black p-4 md:p-6 rounded-lg max-w-md w-full shadow-xl relative">
            <button
              className="absolute top-2 md:top-3 right-2 md:right-3 text-xl md:text-2xl font-bold text-black"
              onClick={() => setIsOpen(false)}
              aria-label="Close contact modal"
            >
              ×
            </button>
            <h2 className="text-xl md:text-2xl font-bold mb-4">Contact Royal Interior</h2>
            <div className="space-y-3 md:space-y-4 text-xs md:text-sm">
              ...
              <a
                ...
                className="inline-block mt-3 md:mt-4 bg-green-500 text-white px-3 md:px-4 py-1.5 md:py-2 rounded hover:bg-green-600 transition"
              >
                Message on WhatsApp
              </a>
            </div>
      ...
      {selectedService && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white text-black p-4 md:p-6 rounded-lg max-w-4xl w-full shadow-xl relative max-h-[90vh] overflow-y-auto">
            <button
              className="absolute top-2 md:top-3 right-2 md:right-3 text-xl md:text-2xl font-bold text-black"
              onClick={closeModal}
            ...
            <h2 className="text-xl md:text-2xl font-bold mb-4 uppercase tracking-[0.1em]">{selectedService.title}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div className="relative">
                ...
                />
                <button
                  className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white/80 p-1.5 md:p-2 rounded-full"
                  onClick={prevImage}
                ...
                <button
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white/80 p-1.5 md:p-2 rounded-full"
                  onClick={nextImage}
              ...
              <div className="space-y-4">
                <p className="text-gray-600 text-xs md:text-sm leading-relaxed">{selectedService.detailedDescription}</p>
              </div>
            </div>
          </div>
        </div>
